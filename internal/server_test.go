...
func TestServerAuthCallback(t *testing.T) {
	...
	req = newHTTPRequest("GET", "http://example.com/_oauth?state="+nonce+":")
	...
	req = newHTTPRequest("GET", "http://example.com/_oauth?state="+nonce+":google:http://redirect.com")
	c = MakeCSRFCookie(req, nonce)
	res, _ = doHttpRequest(req, c)
	require.Equal(401, res.StatusCode, "unauthorized redirect domain should not be allowed")
